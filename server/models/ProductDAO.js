...
const ProductDAO = {
    ...
    async selectByBrandID(_bid) {
        const query = { 'brand._id': _bid };
        const products = await Models.Product.find(query).exec();
        return products;
    },

    async update(product) {
        const newvalues = { name: product.name, price: product.price, image: product.image, category: product.category, brand: product.brand };
